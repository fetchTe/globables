...
# build/output dir
DST = $(CURDIR)/dist
# test(s) (if un/not-defined or dir, recursive test search: bun.sh/docs/cli/test#run-tests)
TES = $(SRC)
...
#: build flags
#! [?1] builds the cjs (CommonJS) target on 'make release'
CJS ?= 1
...
help: ## displays (this) help screen
	@printf "$(BW)#$(CR)$(FW) USAGE$(CR) $(FD)(globables)$(CR)$(NL)"
